...
void SearchServer(char* name)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, (char*)&timeout, sizeof(int)) == SOCKET_ERROR)
    {
    ...
    {
        cout << "There are several servers on the network with the callsign " << name << endl;
        return;
    }
    else
    {
        cout << "There is only one server on the network with a callsign " << name << endl;
    }
}
bool GetServer(char* name, short port, sockaddr* from, int* flen)
{
